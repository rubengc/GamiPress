function gamipress_select2_posts_template_result(item){if(item.post_type!==undefined){var post_type_label=item.post_type;if(gamipress_admin_functions.post_type_labels[item.post_type]!==undefined){post_type_label=gamipress_admin_functions.post_type_labels[item.post_type]}
...
function gamipress_select2_optgroup_matcher(params,data){data.parentText=data.parentText||'';params.term=params.term||'';if($.trim(params.term)===''){return data}
if(data.children&&data.children.length>0){var match=$.extend(!0,{},data);for(var c=data.children.length-1;c>=0;c--){var child=data.children[c];child.parentText+=data.parentText+" "+data.text;var matches=gamipress_select2_optgroup_matcher(params,child);if(matches==null){match.children.splice(c,1)}}
...
show_in_console=!1;try{$('<select id="gamipress-select2-version-check" style="display: none;"><option value=""></option></select>').insertAfter('body');$("#gamipress-select2-version-check").select2({theme:'gamipress-select2-hidden'});$("#gamipress-select2-version-check").select2('isOpen');if(show_in_console)
