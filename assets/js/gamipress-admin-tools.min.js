...
$this.prop('disabled',!1)}})});$("#recount_activity").click(function(e){e.preventDefault();$('#recount-activity-warning').remove();if($('#activity_to_recount').val()===''){$(this).parent().prepend('<p id="recount-activity-warning" class="cmb2-metabox-description" style="color: #a00;">You need to choose an activity to recount.</p>');return!1}
var $this=$(this);var activity=$('#activity_to_recount').val();$this.prop('disabled',!0);$this.parent().prepend('<p id="recount-activity-notice" class="cmb2-metabox-description">'+gamipress_admin_tools.recount_activity_notice+'</p>');$this.parent().append('<span id="recount-activity-response"><span class="spinner is-active" style="float: none;"></span></span>');$.post(ajaxurl,{action:'gamipress_recount_activity_tool',activity:activity},function(response){$('#recount-activity-notice').remove();if(response.success===!1){$('#recount-activity-response').css({color:'#a00'})}
...
$this.prop('disabled',!1)}).fail(function(){$('#recount-activity-response').html('The server has returned an internal error.');setTimeout(function(){$('#recount-activity-notice').remove();$('#recount-activity-response').remove()},5000);$this.prop('disabled',!1)})})})(jQuery)