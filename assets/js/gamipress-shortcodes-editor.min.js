...
if(value!==''&&value!==undefined&&value!==null){key=el.name.replace(shortcode+'_','').replace('[]','');attrs[key]=value}}});$('#'+shortcode+'_wrapper').trigger('gamipress_get_shortcode_attributes',[attrs,inputs]);return attrs}
function gamipress_get_shortcode_inputs(shortcode){return $('input, select, textarea','#'+shortcode+'_wrapper .cmb2-wrap')}
function gamipress_construct_shortcode(shortcode,attributes){var output='[';output+=shortcode;$.each(attributes,function(key,value){output+=' '+key+'="'+value+'"'});$.trim(output);output+=']';return output}
...
var formatted_results=[];results.data.forEach(function(item){formatted_results.push({id:item.ID,text:item.post_title,})});return{results:formatted_results}}},theme:'default gamipress-select2',placeholder:gamipress_shortcodes_editor.id_placeholder,allowClear:!0,multiple:!1};var select2_achievements_multiple=$.extend(!0,{},select2_achievements,{multiple:!0});$('#gamipress_achievement_id').select2(select2_achievements);$('#gamipress_achievements_include, #gamipress_achievements_exclude, #gamipress_logs_include, #gamipress_logs_exclude').select2(select2_achievements_multiple);$('#gamipress_achievements_type, #gamipress_points_types_type, #gamipress_points_type, #gamipress_ranks_type, #gamipress_earnings_points_types, #gamipress_earnings_achievement_types, #gamipress_earnings_rank_types').select2({theme:'default gamipress-select2',placeholder:gamipress_shortcodes_editor.post_type_placeholder,allowClear:!0,multiple:!0});$('#gamipress_achievements_user_id, #gamipress_logs_user_id, #gamipress_points_user_id, #gamipress_rank_user_id, #gamipress_ranks_user_id, #gamipress_earnings_user_id').select2({ajax:{url:ajaxurl,dataType:'json',delay:250,type:'POST',data:function(params){return{q:params.term,action:'gamipress_get_users'}},processResults:function(results,page){if(results===null){return{results:[]}}
var formatted_results=[];results.data.forEach(function(item){formatted_results.push({id:item.ID,text:item.user_login,})});return{results:formatted_results}}},theme:'default gamipress-select2',placeholder:gamipress_shortcodes_editor.user_placeholder,allowClear:!0,multiple:!1});$('#gamipress_achievements_current_user, #gamipress_points_current_user, #gamipress_logs_current_user, #gamipress_ranks_current_user, #gamipress_earnings_current_user').change(function(){var target=$(this).closest('.cmb-row').next();if($(this).prop('checked')){target.slideUp().addClass('cmb2-tab-ignore')}else{if(target.closest('.cmb-tabs-wrap').length){if(target.hasClass('cmb-tab-active-item')){target.slideDown()}}else{target.slideDown()}
target.removeClass('cmb2-tab-ignore')}});var select2_ranks={ajax:{url:ajaxurl,dataType:'json',delay:250,type:'POST',data:function(params){return{q:params.term,action:'gamipress_get_ranks_options'}},processResults:function(results,page){if(results===null){return{results:[]}}
