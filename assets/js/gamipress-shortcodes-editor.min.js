...
function gamipress_get_attributes(shortcode){var attrs={};var inputs=gamipress_get_shortcode_inputs(shortcode);$.each(inputs,function(index,el){var key,value;if($(el).closest('.cmb-row').hasClass('cmb-repeat-row')){key=el.name.replace(shortcode+'_','').replace('[]','');key=key.split('[')[0];if(key==='')return!0;if(attrs[key]===undefined){var field_name=el.name.split('[')[0];var fields=$(el).closest('.cmb-tbody').find('[name^="'+field_name+'"]');var values=[];for(var i=0;i<fields.length;i++){var field=$(fields[i]);if(field.val().length){values.push(field.val())}}
